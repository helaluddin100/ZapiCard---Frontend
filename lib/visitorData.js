...
export async function collectVisitorData() {
    ...
    // Get Browser Geolocation (if available and permitted) - with priority
    // Start geolocation request immediately but don't wait too long
    ...
    // Wait for geolocation (max 5 seconds) before proceeding
...
async function getLocationFromCoordinates(latitude, longitude) {
    try {
        ...
        const response = await fetch(
            ...
            {
                headers: {
                    'User-Agent': 'ZapiCard Visitor Tracking',
