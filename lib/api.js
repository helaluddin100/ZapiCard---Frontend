// API Configuration
// Use runtime base URL in browser so production site always calls production API (fixes iOS "Load failed" when build had localhost)
export function getApiBaseUrl() {
    if (typeof window !== 'undefined') {
        const host = window.location.hostname
        if (host === 'smart.buytiq.store' || host === 'www.smart.buytiq.store' || host.includes('buytiq.store')) {
            return 'https://smart.buytiq.store/api'
        }
    }
